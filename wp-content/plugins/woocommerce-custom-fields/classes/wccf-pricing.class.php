...
class WCCF_Pricing
{
    ...
    private static function get_price_adjusted_by_fields($price, $product_id, $variation_id, $context, $posted = array(), $quantity = 1, $calculate_taxes = false, $rounded = true, $product = null, $is_cart = false)
    {
        ...
        $fields = WCCF_Controller_Conditions::filter_fields($all_fields, array('item_id' => $product_id, 'child_id' => $variation_id));
        ...
        for ($i = 0; $i < $quantity; $i++) {
            ...
            foreach ($fields as $field) {
                ...
                if ($context === 'product_field' && empty($posted)) {
                    if (!WCCF_Controller_Conditions::check_frontend_conditions($field, $fields, $default_values)) {
                        continue;
                    }
                }
                ...
                // Field is quantity based and product is not in cart
                if ($field->is_quantity_based() && !$is_cart) {
    ...
    private static function get_product_field_value_for_price_adjustment($field, $product_id, $product = null, $posted = array(), $quantity_index = null, $is_cart = false)
    {
        ...
        if (WCCF_Settings::get('product_field_prices_include_default') && (!is_object($product) || empty($product->rightpress_in_cart) || RightPress_Product_Price_Shop::is_calculating())) {
     ...
     * @return string
     */
    public static function get_pricing_string($pricing_method, $pricing_value, $is_html = false, $prepend = '', $append = '', $product = null)
    {
        // Get WC config
        ...
        if (self::pricing_method_is_percentage($pricing_method)) {
            ...
            if ($product) {
                ...
                $tax_display = (is_cart() || is_checkout()) ? get_option('woocommerce_tax_display_cart') : get_option('woocommerce_tax_display_shop');

                // Include or exclude tax
                if ($tax_display === 'excl') {
                    $pricing_value = wc_get_price_excluding_tax($product, array('qty' => 1, 'price' => $pricing_value));
                }
                else {
                    $pricing_value = wc_get_price_including_tax($product, array('qty' => 1, 'price' => $pricing_value));
                }
            }

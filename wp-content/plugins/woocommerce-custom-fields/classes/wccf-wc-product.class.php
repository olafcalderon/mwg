...
class WCCF_WC_Product
{
    ...
    // Track which products fields were printed for during current request
    private $fields_printed_for_product_ids = array();
    ...
    public function __construct()
    {
        ...
        // Product field display setup
        add_action('init', array($this, 'product_field_display_setup'));

        // RightPress product pricing system setup
        add_action('init', array($this, 'rightpress_product_pricing_setup'));
     ...
     * @return void
     */
    public function product_field_display_setup()
    {

        $hook_for_product_fields = apply_filters('wccf_product_fields_position_hook', 'woocommerce_before_add_to_cart_button');
        add_action($hook_for_product_fields, array($this, 'display_product_fields'));
    }
    ...
    public function display_product_fields()
    {
        ...
        // Unable to determine product
        if (!is_a($product, 'WC_Product')) {
            return;
        }

        // Ensure fields are not printed multiple times for the same product in case action is called multiple times
        if (in_array($product->get_id(), $this->fields_printed_for_product_ids, true)) {
            return;
        }
        else {
            $this->fields_printed_for_product_ids[] = $product->get_id();
        }
        ...
        $fields = WCCF_Controller_Conditions::filter_fields($all_fields, $filter_params);
        ...
        WCCF_Field_Controller::print_fields($fields, null, $quantity, null, $product);
    ...
    public function ajax_refresh_product_field_view()
    {
        try {
            ...
            $product    = is_a($product, 'WC_Product') ? $product : null;
            ...
            foreach ($fields as $field) {
                for ($i = 0; $i < $data['quantity']; $i++) {
                    ...
                    // Print single field
                    WCCF_Field_Controller::print_fields(array($field), null, $data['quantity'], $quantity_index, $product);
            ...
            if ($group_by_quantity_index) {
                ...
                $response_data = array_merge($not_quantity_based, $response_data);
            }
    ...
    public function add_meta_box_product_prop($post_type, $post)
    {
        ...
        $fields = WCCF_Product_Property_Controller::get_filtered(null, array('item_id' => $post->ID), null, true);
    ...
    public function save_product_property_values($post_id, $post)
    {
        ...
        // Don't update on quick edit (action 'inline-save', not 'editpost')
     ...
     * @return void
     */
    public function rightpress_product_pricing_setup()
    {
        // Check if prices are subject to adjustment
        if (!WCCF_WC_Product::prices_subject_to_adjustment()) {
            return;
        }

        // Product price live update hooks
        if (WCCF_Settings::get('display_total_price')) {
            add_filter('rightpress_product_price_live_update_enabled', '__return_true');
            add_filter('rightpress_product_price_live_update_custom_keys', array($this, 'product_price_live_update_custom_keys'), $this->rightpress_hook_position);
            add_filter('rightpress_product_price_live_update_always_display', '__return_true');
        }
    }

    /**
     * Request field data to be included in product price live update data set
     *
     * @access public
     * @param array $custom_keys
     * @return array
     */
    public function product_price_live_update_custom_keys($custom_keys)
    {

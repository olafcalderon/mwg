...
class Rey_WC_Product_Cat_List_Walker extends Walker {
	...
	public function start_el( &$output, $cat, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		$li_classes = 'rey-cw-item rey-cw-item-' . $cat_id;

		if ( $args['current_category'] === $cat_id ) {
			$li_classes .= ' rey-cw-item-current';
		}

		if ( $args['has_children'] && $args['hierarchical'] && ( empty( $args['max_depth'] ) || $args['max_depth'] > $depth + 1 ) ) {
			$li_classes .= ' rey-cw-item-parent';
		}

		if ( $args['current_category_ancestors'] && $args['current_category'] && in_array( $cat_id, $args['current_category_ancestors'], true ) ) {
			$li_classes .= ' rey-cw-item-current-parent';
		}

		$output .= sprintf('<li class="%s" %s>', $li_classes, $li_attribute);

		// show accordion list icon
		if( $args['has_children'] && $args['hierarchical'] && $args['accordion_list'] ){
